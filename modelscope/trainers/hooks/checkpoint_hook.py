...
def extend_save_name_for_parallel(cur_save_name: str) -> str:
    ...
    try:
        ...
        cur_save_folder, cur_save_name = os.path.dirname(cur_save_name), os.path.basename(cur_save_name)
        cur_save_name = cur_save_name.replace('.', '_mp_rank_{:02d}.'.format(mp_rank))
        cur_save_name= os.path.join(cur_save_folder, cur_save_name)
        return cur_save_name
